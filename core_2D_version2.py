...
def two_dimensional_prediction(x, angle):
    len_x = x.shape[-1]
    x_point1 = np.concatenate([np.full((1, len_x), x[0, 0]), np.full((1, len_x), x[1, 0])], axis=0)
    new_x = x - x_point1
    ...
    final_x = np.dot(angle_matrix_ni, new_x)

    return final_x

# def input_code_angle(number, length):
#     input_total_code = []
#     input_total_angle = []
#
...
def Predict_2D(inputs_total, inputs_angle):
    ...
    for i in range(len(inputs_total)):
        ...
        output_total.append(coordinate_original_points)

    # output_total_angle = []
    for i in range(len(output_total)):
        output_total_i = two_dimensional_prediction(output_total[i], inputs_angle[i])
        # output_total_angle.append(output_total_i)

        plt.plot(output_total_i[0], output_total_i[1], marker='o', linestyle='-', markersize=3)

        x_mean = np.mean(output_total_i[0])
        y_mean = np.mean(output_total_i[1])
        plt.text(x_mean, y_mean, f'{i + 1}', fontsize=10, color='black')
